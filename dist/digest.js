...
export function updateDigest(summary, context) {
...
export function updateTimeline(date, topic) {
    mkdirSync(scribeDir(), { recursive: true });
    ...
    if (idx === -1) {
        ...
        if (merged.length > TIMELINE_DAY_CAP)
            merged = `${merged.slice(0, TIMELINE_DAY_CAP)}…`;
        body[idx] = merged;
    }

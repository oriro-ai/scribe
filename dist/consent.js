...
export function isScribeEnabled() {
    try {
        const f = consentFile();
        if (!existsSync(f))
            return false;
...
export function setScribeConsent(enabled) {
    const f = consentFile();
    mkdirSync(dirname(f), { recursive: true });
    const state = { enabled, consentedAt: new Date().toISOString() };
    writeFileSync(f, `${JSON.stringify(state, null, 2)}\n`, "utf8");
    return state;
}
/** True once the user has been asked (consent file exists), regardless of choice. */
export function hasScribeChoice() {
    try {

...
function oneLineSummary(rec) {
    const bits = [];
    if (rec.user)
        bits.push(rec.user.replace(/\s+/g, " ").slice(0, 80));
    if (rec.files?.length)
        bits.push(`files: ${rec.files.slice(0, 3).join(", ")}`);
    if (rec.note)
        bits.push(rec.note.replace(/\s+/g, " ").slice(0, 60));
...
export function redactRecord(rec) {
    ...
    const rd = (s) => {
        ...
        for (const x of r.redactions)
            tally.set(x.label, (tally.get(x.label) ?? 0) + x.count);
        return r.text;
    };
    const safeRec = {
        ...rec,
...
export function captureTurn(rec) {
